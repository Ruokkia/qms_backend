...
```
POST   /api/v1/trace-nodes            创建追溯节点
GET    /api/v1/trace-nodes?page=&size=&plantCode=&nodeType=&nodeCode=    分页查询
GET    /api/v1/trace-nodes/{id}       查询节点详情
PUT    /api/v1/trace-nodes/{id}       更新节点
DELETE /api/v1/trace-nodes/{id}       逻辑删除
```
...
| 字段 | 类型 | 必填 | 说明 |
|------|------|:--:|------|
| nodeType | string | 是 | 节点类型：SN/部件/关键物料/非关键物料/来料批次/生产批次 |
| nodeCode | string | 是 | 节点编码（SN或物料码，唯一） |
| parentId | long | 否 | 直接父级ID（NULL=根节点） |
| batchId | long | 否 | 关联批次ID |
| qtyUsed | decimal | 否 | 该节点用量（支持 1:N 配比） |
| workOrderId | long | 否 | 关联工单ID |

**约束：** `parentId != id` 防自引用（DB CHECK）；层级上限 8（应用层控制）。
...
| parentId | long | 直接父级ID |
| parentCode | string | 父级编码（连表填充，便于前端展示） |
| batchId | long | 关联批次ID |
| qtyUsed | decimal | 用量 |
| workOrderId | long | 关联工单ID |
| plantCode | string | 分公司编码 |
...
```
GET /api/v1/trace/forward?nodeCode=&maxLevel=8    正向追溯（来料→成品）
GET /api/v1/trace/backward?nodeCode=&maxLevel=8   反向追溯（成品→来料）
GET /api/v1/trace/full?nodeCode=&maxLevel=8       双向全链路
```

| 参数 | 类型 | 必填 | 说明 |
|------|------|:--:|------|
| nodeCode | string | 是 | 起始节点编码（SN/物料码/批次号） |
| maxLevel | int | 否 | 最大层级（默认8，上限8） |
        ...
        "children": [
          {
            ...
            "qtyUsed": 5,
            "level": 3,
            "path": "SN2026-SZ-0001 > M001-SN01 > MC-001",
            "batchInfo": {
              "batchNo": "B-SZ-2026-001",
...
实现方式：PostgreSQL `WITH RECURSIVE` CTE，`child.parent_id = parent.id` 关联，`level <= maxLevel` 截断。前端渲染为 ECharts tree 树状图（默认）+ 列表视图（可切换）。
